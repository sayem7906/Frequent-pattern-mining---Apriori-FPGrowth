...
if __name__ == '__main__':
    ...
    #minSupPerT = [[1.0]]
    open('../Files/result_fpgrowth_53.csv','w').close()
    open("../Files/patterns_fpgrowth_53.csv", 'w').close()
    ct = 0
    for idx,dataset in enumerate(datasets):
        ...
        for minSupPer in minSupPerT[idx]:
            ...
            algo = FpGrowth(minSupPer,inputFile)
            tot_pat,time,memory = algo.algorithm()
            
            #save output
            output_file = '../Files/result_fpgrowth_53.csv'
            title = 'dataset,min_sup %,Total Patterns,Fpgrowth time(s),Fpgrowth memory(MB)\n'
